...
import { countResourcesInGraph, insertFileInDb, getBatchedTriples, isTaskInExpectedJob, updateStatus } from './queries';
import { STATUS_SCHEDULED, STATUS_BUSY, STATUS_SUCCESS, STATUS_FAILED } from './constants';
import { DUMP_FILE_CREATION_JOB_OPERATION } from './env';
...
export async function getScheduledDumpTask(inserts) {
  ...
  const taskInExpectedJob = await isTaskInExpectedJob(task, DUMP_FILE_CREATION_JOB_OPERATION);
...
export async function produceDumpFile(task) {
  try {
    ...
    console.log(`An error occured while creating dump file for task ${task}: ${e}`)
...
*/
async function generateDumpFile(file) {
...
async function saveTriplesToFile(file, count) {
  ...
  while (offset < count) {
    ...
    const serializedTriples = triples.map(t => serializeTriple(t)).join('\n');

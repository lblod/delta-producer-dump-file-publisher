...
*/
export async function generateDumpFile( fileBaseName, graphToDump ) {
  ...
  const count = await countResourcesInGraph(graphToDump);
  ...
  if(count == 0){
    ...
    await saveTriplesToFile(tmpFile, count, graphToDump);
    await fs.rename(tmpFile, ttlFile);
    return ttlFile;
  }
}

async function saveTriplesToFile(file, count, graph) {
  let offset = 0;
  while (offset < count) {
    const triples = await getBatchedTriples(offset, graph);

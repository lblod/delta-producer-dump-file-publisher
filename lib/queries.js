...
import { STATUS_SCHEDULED, ERROR_URI_PREFIX, ERROR_TYPE } from './constants';
import {
  FILES_GRAPH,
...
export async function isTaskInExpectedJob(task, type) {
  const q = `
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    PREFIX dct: <http://purl.org/dc/terms/>
    ...
    SELECT ?job
    WHERE {
      ${sparqlEscapeUri(task)} dct:isPartOf ?job .
      ?job task:operation ${sparqlEscapeUri(type)} .
    }
    LIMIT 1
  `;

  const queryResult = await query(q);
  return queryResult.results.bindings.length;
}

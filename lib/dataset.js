...
import { DCAT_DATASET_SUBJECT, DCAT_DATASET_GRAPH, SERVICE_NAME, FILES_GRAPH } from './env';
...
export class DatasetManager {
  ...
  async createDumpFile() {
    this.filePath = await generateDumpFile();
    await this.generateDataset();
    await this.generateTtlDistribution();
    await this.deprecatePrevious();
    //TODO: a clean up of the previous files to avoid explostion of dumps
  }
      ...
      INSERT DATA {
        GRAPH ${sparqlEscapeUri(DCAT_DATASET_GRAPH)} {
          <${uri}> a dcat:Dataset ;
            ...
            dct:subject ${sparqlEscapeUri(DCAT_DATASET_SUBJECT)};

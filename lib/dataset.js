...
import { DCAT_DATASET_GRAPH, SERVICE_NAME, FILES_GRAPH, CLEAN_UP_OLD_DUMPS } from './env';
...
export class DatasetManager {
  constructor(dcatDataSetSubject, graphToDump, fileBaseName, publicationGraphEndpoint, targetDcatGraph, targetFilesGraph, cleanupOldDumps){
  ...
  async deletePrevious() {
    const datasets = this.getPreviousDataset();
    if (datasets.length == 1) {
      const {dataset, distribution} = datasets[0];

      const queryFileNameAndUri = `
        ${PREFIXES}

        SELECT DISTINCT ?logicalFileUri ?fileName
        where {
          GRAPH ${sparqlEscapeUri(this.dcatGraph)} {
      ...
      `;
      const results = await query(queryFileNameAndUri);
      const entries = results.bindings ? results.bindings.map(d => {
        return {logicalFileUri: d.logicalFileUri.value, fileName: d.fileName.value};
      }) : [];

      for (const entry of entries) {
        await deleteDumpFile(entry.fileName);
        const deleteFileQuery = `
          ...
          DELETE {
            ...
            BIND(${sparqlEscapeUri(entry.logicalFileUri)} as ?logicalFileUri)
            GRAPH ${sparqlEscapeUri(this.filesGraph)} {
              ?physicalFileUri a nfo:FileDataObject ;
                               nie:dataSource ?logicalFileUri.
               ?physicalFileUri ?p1 ?o1.
      ...
      DELETE {
        GRAPH ${sparqlEscapeUri(this.dcatGraph)} {
          ${sparqlEscapeUri(this.datasetUri)} prov:wasRevisionOf ${sparqlEscapeUri(dataset)} .
          ${sparqlEscapeUri(dataset)} dcat:distribution ?distribution.
          ?distribution ?p ?o.
        }
      } WHERE {

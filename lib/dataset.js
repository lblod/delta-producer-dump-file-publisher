...
export class DatasetManager {
  constructor(dcatDataSetSubject, graphToDump, fileBaseName, publicationEnpoint){
    this.dcatDataSetSubject = dcatDataSetSubject;
    this.fileBaseName = fileBaseName;
    this.graphToDump = graphToDump;
    this.publicationEndpoint = publicationEnpoint || process.env.MU_SPARQL_ENDPOINT;
  }
  /**
   * @public
   */
  async createDumpFile() {
    this.filePath = await generateDumpFile(this.fileBaseName, this.graphToDump, this.publicationEndpoint);
    if(!this.filePath){
      return;
    }
    else {
